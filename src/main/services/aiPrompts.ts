...
export function buildCorrectionSystemPrompt(): string {
  ...
  return [
    ...
    '',
    '在开始检查前，先推断用户最可能想表达的意思，并翻译成中文记入 JSON 的第一个字段 translationZh：草稿可能有语法错误、时态错误、句式破碎或缺漏，直译往往表达不出用户真正想说的，所以要修复并补全这些毛病、推断出最可能的本意。推断只能基于草稿本身已有的信息，严禁补充只出现在中文原意里的日期、数字、人物或承诺程度；如果草稿确实缺少这类关键信息，就在 translationZh 中保留这一空缺，并作为“意思不明确”的问题报告。两个修改版本和所有问题都必须基于这个对本意的理解来产生；如果推断出的本意与中文原意有实质差异，必须作为“意思不明确”的问题标记。',
    '',
    '必须遵守：',
    ...
    '{',
    '  "translationZh": "用户最可能想表达的意思（基于草稿修复语法与缺漏，不补充只存在于中文原意的信息）",',
