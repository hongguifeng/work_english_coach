...
- [x] 注册打开应用快捷键（应用运行期间按下 → 显示/恢复/聚焦主窗口，并跳到工作区页面；默认 Ctrl+Shift+Space）。
- [x] 检查快捷键冲突（globalShortcut.register 返回 false 时标记失败，不崩溃）。
- [x] 快捷键失败时显示提示（设置页红色 Tag 展示失败原因）。
- [x] 支持设置中自定义修改或关闭（快捷键用「按键录制」方式修改：点「重新录制」后直接按组合键（需含 Ctrl/Alt/Win 修饰键，Esc 取消），松开即转为 Electron accelerator 保存并重新注册；另有启用开关；注册失败给明确提示）。
- [x] 注销快捷键时清理资源（will-quit 时 unregisterAll）。
...
- [x] 首次安装测试。（NSIS 安装包 `release/WorkEnglish Coach Setup 0.1.0.exe` 构建成功；当前环境无管理员权限，无法静默安装到 Program Files，改用 `--dir` 展开包 + 程序目录替换/删除模拟安装/卸载语义）
- [x] 首次启动测试。（全新临时 DB：窗口挂载、表达库/错误档案空状态、AI 配置默认空）
- [x] 配置 API Key 测试。（keytar → Windows 凭据管理器，toast 确认，未写入 DB/日志）
- [x] AI 纠错测试。（本地 qwen 测试服务器：最小修改版/自然版/问题分类返回并渲染）
- [x] 数据保存测试。（确认并保存 → samples/issues/skills/expressions 落库；手动加表达落库）
- [x] 重启测试。（杀进程后重启：数据、AI 配置、凭据全部存活，重启后再次 AI 调用成功）
- [x] 数据导出测试。（生产 DB → `wec-export/v1` JSON：六表 + settings 计数正确，内容不含 API Key；保存对话框无法 CDP 操作，文件写入部分以“点击后进程存活 + 内容层导出”替代验证）
- [x] 数据删除测试。（设置页删除全部数据：六张学习表清空、settings 保留、UI 刷新）
- [x] 升级或重新安装测试。（程序目录整体替换为新版构建后启动，数据完整）
- [x] 卸载行为测试。（删除程序目录后 userData 数据文件仍在且完整；重装后数据恢复可见）

> 完成记录（2026-04）：`tmp-t040-prod.mjs` 生产包 CDP 全流程 32/32 通过。局限：无管理员权限，未实际运行 NSIS 静默安装/卸载（安装物本身已验证生成；安装/卸载语义用程序目录等价操作验证）。

---

## T041：补充用户文档

- [x] 编写安装说明。（`docs/10-user-guide.md` §1）
- [x] 编写 API 配置说明。（§2，含 Key 的存储位置与清除方式）
- [x] 编写隐私说明。（§3，本地数据/凭据/不读他应用/录音权限/AI 供应商边界）
- [x] 编写数据目录说明。（§4，`%APPDATA%\WorkEnglish Coach\work-english-coach\`）
- [x] 编写故障排查说明。（§5，超时/JSON 无效/认证/麦克风/DB 损坏/开发端口）
- [x] 编写 AI 服务兼容性说明。（§6，OpenAI chat/completions 契约/已知可用端点/换模型注意）
